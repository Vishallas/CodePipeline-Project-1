...
GitHub push of git tag
  ...
  EventBridge Rule  (one rule per PG major — created by Terraform)
        ...
        ▼
  CodePipeline V2  (pg-platform-pkg-pg{N} — created by Terraform)
        │
...
Watch: **AWS Console → CodePipeline → pg-platform-pkg-pg14** — stages should turn green.
...
| Parameter | Value |
|-----------|-------|
| `/pg-platform/cicd/build/S3_BUCKET` | Artifacts bucket name |
| `/pg-platform/cicd/build/BUILD_ENV` | `staging` |
| `/pg-platform/cicd/ecr/account_id` | AWS account ID |
| `/pg-platform/cicd/ecr/region` | AWS region |
| `/pg-platform/cicd/gpg/key_id` | GPG key fingerprint |
| `/pg-platform/cicd/cloudfront/distribution_id` | CloudFront dist ID |

### Dynamic (written by ParseTag CodeBuild on every pipeline run)

| Parameter | Written by |
|-----------|-----------|
| `/pg-platform/cicd/build/PACKAGE_NAME` | `buildspec-parse-tag.yml` |
| `/pg-platform/cicd/build/PACKAGE_VERSION` | `buildspec-parse-tag.yml` |
| `/pg-platform/cicd/build/PACKAGE_REVISION` | `buildspec-parse-tag.yml` |
| `/pg-platform/cicd/build/PG_MAJOR` | `buildspec-parse-tag.yml` |
| `/pg-platform/cicd/build/GIT_TAG` | `buildspec-parse-tag.yml` |
| `/pg-platform/cicd/build/IS_RC` | `buildspec-parse-tag.yml` |
...
| Secret | Content |
|--------|---------|
| `pg-platform/cicd/gpg-signing-key` | Base64-encoded GPG private key |
| `pg-platform/cicd/pulp-password` | Pulp API password (only if `pulp_url` is set) |

---

## ParseTag Stage — How It Works

Stage 2 is a CodeBuild project (`pg-platform-parse-tag`), **no Lambda required**.
...
5. Writes 6 values to SSM `/pg-platform/cicd/build/*` (overwrite)
...
EventBridge detects the tag and starts `pg-platform-pkg-pg14` within ~1 minute.
...
**CodePipeline → pg-platform-pkg-pg14 → Release change.**
...
[Tag pushed: pg14/postgresql-14-14.21-1]
    │
    ▼
EventBridge rule pg-platform-trigger-pg14 fires (~1 min after push)
    │  Injects: variables[GIT_TAG] = "pg14/postgresql-14-14.21-1"
    ▼
Stage 1: Source
    ├─ packages_source: checkout pg-packaging @ pg14
    └─ platform_source: checkout pg-platform @ main
...
Stage 2: ParseTag  (~1-2 min)
    ...
    ├─ Writes 6 SSM params under /pg-platform/cicd/build/
...
Stage 5: UpdateStagingRepo  (~3 min)
    reprepro → APT staging index → s3://pg-platform-apt-repo/staging/
    createrepo_c → YUM staging index → s3://pg-platform-yum-repo/staging/
...
aws ssm get-parameters \
  --names /pg-platform/cicd/build/S3_BUCKET \
...
aws codebuild batch-get-projects \
  --names pg-platform-parse-tag pg-platform-build-ubuntu-amd64 pg-platform-repo-updater \
  --query 'projects[*].[name,created]' --output table

# 5. Confirm pipelines
aws codepipeline list-pipelines \
  --query 'pipelines[?starts_with(name,`pg-platform-pkg`)].name' --output table

# 6. Confirm EventBridge rules
aws events list-rules --name-prefix pg-platform-trigger \
...
| Stage 2 fails — SSM write denied | CodeBuild role missing `ssm:PutParameter` on `/pg-platform/cicd/build/*` | `terraform apply` re-syncs IAM policy |
...
| `terraform apply` fails — SSM conflict | Param exists with different type | Import: `terraform import aws_ssm_parameter.s3_bucket /pg-platform/cicd/build/S3_BUCKET` |
...
Manual (one-time):
  ...
  [ ] GPG key value uploaded to Secrets Manager (pg-platform/cicd/gpg-signing-key)
